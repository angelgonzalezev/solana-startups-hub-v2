...
export default function VerificationPage({ params }: { params: { id: string } }) {
  ...
  const getStatusMessage = () => {
    ...
    switch (startup.verificationStatus) {
      case 'draft': return 'Complete your startup profile and request verification.';
      case 'pending': return 'Verification is pending. We are reviewing your website and X profile.';
      case 'verified': return 'Your startup is verified and can be published.';
      case 'rejected': return 'Verification was rejected. Update the required information and request verification again.';
      default: return '';
    }
  };
          ...
          <div className="max-w-4xl space-y-8">
            ...
            <div className="bg-[#0A0A0A] p-8 md:p-10 border border-white/5 rounded-[30px] space-y-8">
                ...
                <div className="space-y-4">
                    ...
                    <CheckItem label="Full Description (min 200 chars)" isDone={!!startup.description && startup.description.length >= 200} />
                    <CheckItem label="Valid Website URL" isDone={!!startup.website} />
                    <CheckItem label="Valid X (Twitter) URL" isDone={!!startup.twitter} />
                    <CheckItem label="Categories & Tech Stack" isDone={startup.category.length > 0 && startup.techStack.length > 0} />
                </div>
            ...
            <div className="bg-white/5 p-8 md:p-10 border border-white/10 rounded-[30px] border-dashed space-y-6">
                ...
                <div className="flex flex-wrap gap-4">
                    <button onClick={handleMockApprove} disabled={isActionLoading} className="btn btn-white-dark btn-md border-green-500/20 hover:bg-green-500/10 text-green-500">
                        Mock Approve
                    </button>
                    <button onClick={handleMockReject} disabled={isActionLoading} className="btn btn-white-dark btn-md border-red-500/20 hover:bg-red-500/10 text-red-500">
                        Mock Reject
                    </button>
                </div>
                <p className="text-white/30 text-xs">These actions are only for development/demo purposes to simulate a reviewer approving or rejecting the startup.</p>
            </div>
...
const CheckItem = ({ label, isDone }: { label: string, isDone: boolean }) => (
    <div className="flex items-center gap-4">
        ...
        <span className={cn(
            "text-lg",
            isDone ? "text-white/80" : "text-white/30"
        )}>
            {label}
        </span>
    </div>
