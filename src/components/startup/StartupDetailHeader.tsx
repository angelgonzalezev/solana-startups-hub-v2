...
const StartupDetailHeader: React.FC<StartupDetailHeaderProps> = ({ startup }) => {
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8 text-center sm:text-left">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xl md:text-2xl text-white/70 max-w-[700px] leading-relaxed">
              {startup.oneLiner}
            </p>
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3">
              <MarketSignalBadge type="raising" />
        ...
        {isOwner && (
          <div className="flex justify-center lg:justify-end">
            <Link
              ...
              className="btn btn-white-dark btn-xl hover:btn-primary border-white/10"
            >
              Edit Startup
            </Link>
          </div>
      ...
      {/* Social & Web Links */}
      <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 py-6 border-y border-white/5">
        {startup.website && (
          <a href={startup.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl transition-all group">
            ...
            <span className="text-sm font-bold text-white/70 group-hover:text-white uppercase tracking-widest">Website</span>
          </a>
        )}
        {startup.twitter && (
          <a href={startup.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl transition-all group">
            <svg className="h-4 w-4 fill-white/40 group-hover:fill-white" viewBox="0 0 24 24">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
            <span className="text-sm font-bold text-white/70 group-hover:text-white uppercase tracking-widest">Twitter</span>
          </a>
        )}
        {startup.github && (
          <a href={startup.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl transition-all group">
            ...
            <span className="text-sm font-bold text-white/70 group-hover:text-white uppercase tracking-widest">GitHub</span>
          </a>
        )}
        {startup.discord && (
          <a href={startup.discord} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl transition-all group">
            ...
            <span className="text-sm font-bold text-white/70 group-hover:text-white uppercase tracking-widest">Discord</span>
          </a>
        )}
      </div>
    </div>
