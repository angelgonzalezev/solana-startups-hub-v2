  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && formData.id) {
        result = await startupService.updateStartup(
          formData.id,
          walletAddress,
          formData
        );
      ...
      setMessage({
        type: 'error',
        text: error.message || 'Failed to save startup.',
      });
    } finally {
    ...
    <form onSubmit={handleSubmit} className="space-y-12">
      <div className="bg-[#0A0A0A] p-8 md:p-10 border border-white/5 rounded-[30px] space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* Description */}
          <div className="col-span-1 md:col-span-2 space-y-2">
            <label className="text-sm font-medium text-white/60 ml-1">Description (200-2000 chars for verification)</label>
            <textarea
              name="description"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
                <div className="flex items-center space-x-4">
                     <button
                        type="button"
                        onClick={() => setFormData(prev => ({ ...prev, acquisitionStatus: prev.acquisitionStatus === 'open_to_discuss' ? 'not_open' : 'open_to_discuss' }))}
                        className={cn(
                            "w-12 h-6 rounded-full transition-all duration-300 relative",
                    ...
                    <div className="space-y-0.5">
                        <label className="text-lg font-bold text-white cursor-pointer" onClick={() => setFormData(prev => ({ ...prev, acquisitionStatus: prev.acquisitionStatus === 'open_to_discuss' ? 'not_open' : 'open_to_discuss' }))}>
                            Open to Acquisition
                        </label>
